...
$conn = new mysqli("localhost", "root", "stunt5800.", "foodkart");
	...
	$filter = $top+$till;
	// var_dump($filter);
	//$query="SELECT p_id,p_name,p_description,p_image_id,p_price,p_location,o_id,o_delivery FROM orders INNER JOIN products ON orders.o_product_id=products.p_id where o_id<=".$filter." and o_user=".$user." ";



	$query="SELECT p_id,p_name,p_description,p_image_id,p_price,p_location,o_id,o_delivery,u_phone FROM ((orders INNER JOIN products ON orders.o_product_id=products.p_id) INNER JOIN users ON products.p_user = users.u_name) where o_id<=".$filter." and o_user=".$user." ";
